...
public class ArduinoThread extends Thread{
	...
	public Handler arduinoHandler = new Handler(Looper.getMainLooper()) {
		...
		public void handleMessage(Message msg) {
			...
			if(myBundle.containsKey("COMMAND") && myBundle.getString("COMMAND") != null){
				String cmd = myBundle.getString("COMMAND");
				synchronized(this){	
					write(cmd);
				}
			}
            if(myBundle.containsKey("COORDINATES") && myBundle.getString("COORDINATES") != null){
                String coordinates = myBundle.getString("COORDINATES");
                synchronized(this){
                    sendGPSData(coordinates);
                }
            }
		}
	};
	...
	protected void write(String cmd) {
		...
		if(cmd.contentEquals("p")){
			...
			try {
				...
				//Log.v(TAG, "Sending ping num. "+pingFrameSeqNum+"--"+aux);
    ...
    /**
     * Sends a command to the connected Arduino via Bluetooth socket
     * @param coordinates: Command to send to the Arduino
     */
    protected void sendGPSData(String coordinates) {
        pingSentTimes.put(pingFrameSeqNum, System.currentTimeMillis());
        List<Byte> outBuffer = ArduinoMessage.constructNewMessage(ArduinoMessage.MSGID_DATA, 0, coordinates);
        ...
        try {
            String aux = Arrays.toString(auxBuff);
            //Log.v(TAG, "Sending ping num. "+pingFrameSeqNum+"--"+aux);
            Log.v(TAG, "Sending data to "+m_devName+": \n"+aux);
            m_outStream.write(auxBuff, 0, outBuffer.size());
        } catch (IOException e) {
            Log.e(TAG, "Exception writting to the Arduino socket");
            e.printStackTrace();
